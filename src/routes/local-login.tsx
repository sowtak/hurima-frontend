...
const LocalLoginForm = () => {
  ...
  const handleFieldChange = (fieldName: keyof LocalLoginFormData) => (
  ...
  const sendAuthToken = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(API_BASE_URL_DEV + '/auth/local-email-login', formData);
  ...
  return (
    <>
      ...
      <FormContainer formName='Local login to Hurima'>
